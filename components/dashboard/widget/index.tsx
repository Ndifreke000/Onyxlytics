...
export default function Widget({ widgetData }: WidgetProps) {
  ...
  const [tps, setTps] = useState("2,847")
  ...
  useEffect(() => {
    const fetchNetworkStatus = async () => {
      try {
        ...
        const data = (await response.json()) as { success: boolean; data?: { validatorCount?: number } }

        if (data.success) {
          setNetworkStatus("HEALTHY")
        }
      } catch (error) {
        console.error("[v0] Failed to fetch network status:", error)
  ...
  return (
    <Card className="w-full aspect-[2] relative overflow-hidden">
      ...
      <CardContent className="bg-accent/30 flex-1 flex flex-col justify-between text-sm font-medium uppercase relative z-20">
        ...
        <div className="flex justify-between items-center">
          <span className="opacity-50">{networkStatus}</span>
